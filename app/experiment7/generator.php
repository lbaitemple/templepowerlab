...
<html>
	...
	new_Speed=0;
	temp=1;
		$('.js-b.resistor .up').attr('disabled', true);
						...
						function timer() {
							...
							console.log("speed is "+value_1);
							console.log(speedometer.value());
							$generator.trigger( "updateGenerator", [value_1/1000] );
							if(value_1!=speedometer.value())
								speedometer.animatedUpdate(value_1, 300);
							//rotate machine
							...
							if (count <= 0) {
								clearInterval(counter);
								$('.js-b.resistor .up').attr('disabled', false);
...
//motor model in discrete time
	...
	var iter = 0;
		...
		//t = t.toFixed(3);
		//index=parseInt(t/0.1);
		if(Speed>2200){alert("Action denied: the speed cannot exceed 125%.");return;}
...
//motor model in discrete time
	...
	Speed = Control_S[temp-1];
		w = Control_w[temp-1];
	new_Speed= Speed;
		 //var tt=temp*span;
   		 //Control_R[temp-1]=tt.toString();
    	I_a = Control_Ia[temp-1];
    	I_f = Control_If[temp-1];
	var $generator = $('#generatorCanvas');
  			var turbine_speed =  $('.js-turbine-speed-indicator').attr('data-speed');
   			$ammeter1 = $('.js-a.ammeter');
    		$ammeter2 = $('.js-f.ammeter');
