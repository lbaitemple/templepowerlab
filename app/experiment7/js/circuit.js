...
$(function(){
    ...
    var addComponentChosen = function($components, $node, component, $options, $heading, $variable_resistor, $window_wrap, $window, hotspot) {
        ...
        if (component === 'ammeter') {
           ...
           if(hotspot=='d')
            $node.html(string+'<span class="resistor_value" id="value_d">' + MAX_RESISTOR_VALUE + ' Ohms</span>');
            if(hotspot=='b')
            $node.html(string+'<span class="resistor_value" id="value_b">' + 0 + ' Ohms</span>');
            //addResistorControlListeners($node, hotspot);
    		...
    		if(location===key)
    		{
    			 ...
    			 if(component===value)
    			 {
					...
					reminder=0;
    			 	...
    			 	reminder++;
    			 	if(reminder===5){
    			 		toastr.warning("<div>Sorry! You placed the component<b style='color:red'><i> incorrectly 5 times</i></b>!</div><a href='../captivate/callees/EXP7/' target='_top' class='btn btn-success'>Review slides</a> <button type='button' id='surpriseBtn' class='btn' style='margin: 0 8px 0 8px'>Close</button></div>");
    			 		reminder=0;
    			 	}else{
  ...
  "hideMethod": "fadeOut"
    ...
    var getResistorControlsHTML = function(orientation, hotspot) {
        ...
        if(hotspot=='b')
        {html += '<button class="resistor-button-b up js-resistor-button" data-type="up" disabled onclick="bringUp()">+</button>';
        html += '<button class="resistor-button-b down js-resistor-button" data-type="down" disabled onclick="bringDown()">-</button>';
        }else if(hotspot=='d')
        html += '<button class="resistor-button-a down js-resistor-button" data-type="down" disabled onclick="cutDown()">cut down</button>';
        html += '</div>';
