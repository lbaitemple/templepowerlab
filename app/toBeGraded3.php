...
<?php // here comes your delete query: use $_POST['deleteItem'] as your id
	...
	$result = mysqli_query($con, "select * FROM exp3 WHERE id='$a'");
...
----Experiment3 Needs to be Graded </h3>
<div class="col-lg-4 " >
...
<li>
Plot the calculated terminal voltage and the actual terminal voltage as a function of the load current.  Are the two plots same?  If not, discuss why not.  Also discuss the overall load voltage profile.
...
<li>
	Were there any visible sparks at the brushes?  Did the sparks increase or decrease with load?   Discuss any possible reason for these sparks.
...
$results = json_decode($var, true);
if ($results[3]['qarray'][1] == null) {echo "No Answer Yet!";
} else
	echo $results[3]['qarray'][1];
...
$results = json_decode($var, true);
if ($results[3]['qarray'][2] == null) {echo "No Answer Yet!";
} else
	echo $results[3]['qarray'][2];
...
$results = json_decode($var, true);
if ($results[3]['qarray'][3] == null) {echo "No Answer Yet!";
} else
	echo $results[3]['qarray'][3];
...
data :<?php // here comes your delete query: use $_POST['deleteItem'] as your id
	...
	echo "[";
	for ($i = 0; $i < count($results[1]['varray']); $i++) {
...
echo "data : ";
	//varray1
	echo "[";
	for ($i = 0; $i <count($results[2]['varray1']); $i++) {
