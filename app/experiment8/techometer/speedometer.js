...
function Speedometer(Element) {
  ...
  var MaxValue = options.max   || 2000.0;
  var CurValue = options.value || MinValue;

  // Threshold
  var Threshold   = options.threshold      || 800.0;
...
function Torquemeter(Element) {
  ...
  var MaxValue = options.max   || 500;
  var CurValue = options.value || MinValue;

  // Threshold
  var Threshold   = options.threshold      || 200;
  var ThreshPivot = options.thresholdPivot || 35;
  ...
  this.drawMeter = function ()
  {
    ...
    for (i = 0; i <= MeterTicksCount; i++)
    {
      ...
      // Draw string and increment ruler value
       drawString (MinValue + (incValue * i), {
        angle: currentAngle,
        color: Color.meter.strings,
        offset: Size / 10,
        size: Size / 23
