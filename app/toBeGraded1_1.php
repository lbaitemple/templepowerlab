...
<?php // here comes your delete query: use $_POST['deleteItem'] as your id
	...
	$result = mysqli_query($con, "select * FROM exp1_1 WHERE id='$a'");
...
----Experiment1_1 Needs to be Graded </h3>
<div class="col-lg-4 " >
...
<label>Score:</label>
<input name="score" type="text" cid="score" placeholder="Score">/40

<a type="submit" name="Submit" href="score1_1.php?deleteItem='.$row['login'].'">Submit Score</a>
...
$results = json_decode($var, true);
if ($results[1]['qarray'][0] == null) {echo "No Answer Yet!";
} else
	echo $results[1]['qarray'][0];
...
$results = json_decode($var, true);
if ($results[1]['qarray'][1] == null) {echo "No Answer Yet!";
} else
	echo $results[1]['qarray'][1];
...
$results = json_decode($var, true);
if ($results[1]['qarray'][2] == null) {echo "No Answer Yet!";
} else
	echo $results[1]['qarray'][2];
...
$results = json_decode($var, true);
if ($results[1]['qarray'][3] == null) {echo "No Answer Yet!";
} else
	echo $results[1]['qarray'][3];
...
$results = json_decode($var, true);
if ($results[1]['qarray'][4] == null) {echo "No Answer Yet!";
} else
	echo $results[1]['qarray'][4];
