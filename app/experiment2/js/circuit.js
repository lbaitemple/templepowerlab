    ...
    var addComponentChosen = function($components, $node, component, $options, $heading, $variable_resistor, $window_wrap, $window, hotspot) {
        ...
        if (component === 'ammeter') {
           ...
           // $('.js-circuit-hotspot.'+hotspot).html(getResistorControlsHTML());
            ...
            $node.html('<span class="resistor_value"'+' id="resistor-'+hotspot+'">' + MAX_RESISTOR_VALUE + ' Ohms</span>');
    		...
    		if(location===key)
    		{
    			 ...
    			 if(component===value)
    			 {
					...
					reminder=0;
    			 	...
    			 	reminder++;
    			 	if(reminder===5){
    			 		toastr.warning("<div>Sorry! You placed the component<b style='color:red'><i> incorrectly 5 times</i></b>!</div><a href='captivate/callees/experiment2/' target='_top' class='btn btn-success'>Review slides</a> <button type='button' id='surpriseBtn' class='btn' style='margin: 0 8px 0 8px'>Close</button></div>");
    			 		reminder=0;
    			 	}else{
...
/******************************************* TURBINE BUTTONS *******************************************/
    ...
    var addVariableResistor = function($heading, $resistor, $window_wrap, $window, $node) {
        ...
        document.onkeydown = function(e) {
	    	switch (e.keyCode) {
	        	case 37:
	         //   alert('left');
	            	moveVarsistorTerminal($terminal, $value_display, $node, factor, offset_x, init_margin_left);
	            	if(factor>=250)
	            		factor=factor-2.5;
    ...
    var moveVarsistorTerminal = function($terminal, $value_display, $node, pageX, offset_x, init_margin_left){
        ...
        last_value= parseInt(last_value/10)*10;
