...
$(function(){
    ...
    var addComponentChosen = function($components, $node, component, $options, $heading, $variable_resistor, $window_wrap, $window, hotspot) {
        ...
        if (component === 'ammeter') {
            ...
            //$node.html('<span class="resistor_value">' + MAX_RESISTOR_VALUE + ' Ohms</span>');
            //$components.hide();
            //addVariableResistor($heading, $variable_resistor, $window_wrap, $window, $node);
        }
    ...
    var checkCompoent= function(component,hotspot){
    	...
    	var location="js-"+hotspot;
    		...
    		var exp = circuit_checker.experiments[Exp].connections;
    		...
    		if(location===key)
    		{
    			 ...
    			 if(component===value)
    			 {
					...
					reminder=0;
    			 	...
    			 	reminder++;
    			 	if(reminder===5){
    			 		toastr.warning("<div>Sorry! You placed the component<b style='color:red'><i> incorrectly 5 times</i></b>!</div><a href='../captivate/callees/experiment4/' target='_top' class='btn btn-success'>Review slides</a> <button type='button' id='surpriseBtn' class='btn' style='margin: 0 8px 0 8px'>Close</button></div>");
    			 		reminder=0;
    			 	}else{
    			 	toastr.warning("<div>Sorry! You placed the component <i style='color:red'><b>incorrectly!</b></i></div><a href='#' class='btn btn-success' data-toggle='modal' data-target='#basicModal' id='okBtn'>Show schematic</a> <button type='button' id='surpriseBtn' class='btn' style='margin: 0 8px 0 8px'>Close</button></div>");
    			 }
    			 }
    		}
  ...
  "hideMethod": "fadeOut"
    ...
    var getResistorControlsHTML = function(orientation, hotspot) {
        ...
        html += '<button class="resistor-button down js-resistor-button" data-type="down" disabled>-</button>';
        html += '</div>';
    ...
    var addResistorControlListeners = function($node, hotspot) {
        ...
        $buttons.bind(eventTypeDown, function(){
            ...
            if ($button.is(':enabled')) {
                circuit_checker.experiments[Exp].calculate(button_type, 'js-'+hotspot);
                //circuit_checker.experiments['exp2'].calculate(button_type, 'js-'+hotspot);
            }
        });
    };
    ...
    var checkIfCircuitIsCorrect = function() {
        if (circuit_checker.checkCircuit(Exp)){
            ...
            circuit_checker.enableResistorControls(Exp);
            circuit_checker.experiments[Exp].calculate();
        }
        ...
        //else circuit is still not correct, make sure everything is zeroed out
            ...
            $('.js-circuit-connection.voltmeter').html('0.00V');
            $turbine_speed_indicator.attr('data-speed', 0);
        };
    };
     var zoom = function(Experiment) {
     if(Experiment=='exp1'){
     		var $node = $('.js-b');
              $node.addClass('zoom');
              var $node = $('.js-h');
    ...
    var disablehotspot = function(Experiment) {
    	
    	document.getElementById("spot-b").className =
   		document.getElementById("spot-b").className.replace
        ...
        document.getElementById("spot-h").className =
   		document.getElementById("spot-h").className.replace
        ( /(?:^|\s)circuit-hotspot(?!\S)/g , '' );
    	document.getElementById("spot-h").className =
   		document.getElementById("spot-h").className.replace
        ( /(?:^|\s)f(?!\S)/g , '' );
