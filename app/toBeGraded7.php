...
<?php // here comes your delete query: use $_POST['deleteItem'] as your id
	...
	$result = mysqli_query($con, "select * FROM exp7 WHERE id='$a'");
...
----Experiment7 Needs to be Graded </h3>
<div class="col-lg-4 " >
...
<label>Score:</label>
<input name="score" type="text" cid="score" placeholder="Score">/100

<a type="submit" name="Submit" href="score6.php?deleteItem='.$row['login'].'">Submit Score</a>
...
<?php // here comes your delete query: use $_POST['deleteItem'] as your id
	...
	for ($i = 0; $i < count($results[0]['fcarray']); $i++) {
		echo "<tr><td><p style='background-color:#FFFF00'>";
		echo 50*($i+1);
		echo "</p>";
		echo "</td><td><p style='background-color:#FFFF00'>";
		echo $results[0]['fcarray'][$i];
		echo "</p>";
		echo "</td><td><p style='background-color:#FFFF00'>";
		echo $results[1]['acarray'][$i];
		echo "</p>";
		echo "</td><td><p style='background-color:#FFFF00'>";
		echo $results[2]['speed'][$i];
		echo "</p>";
		echo "</td><td><p style='background-color:#FFFF00'>";
		echo $results[3]['cspeed'][$i];
		echo "</p></td></tr>	";
	}//echo "<script type='text/javascript'>alert('111');</script>";
	mysqli_close($con);
?>
...
$results = json_decode($var, true);
if ($results[4]['qarray'][0] == null) {echo "No Answer Yet!";
} else
	echo $results[4]['qarray'][0];
...
$results = json_decode($var, true);
if ($results[4]['qarray'][1] == null) {echo "No Answer Yet!";
} else
	echo $results[4]['qarray'][1];
...
$results = json_decode($var, true);
if ($results[4]['qarray'][2] == null) {echo "No Answer Yet!";
} else
	echo $results[4]['qarray'][2];
...
$results = json_decode($var, true);
if ($results[4]['qarray'][3] == null) {echo "No Answer Yet!";
} else
	echo $results[4]['qarray'][3];
...
data :<?php // here comes your delete query: use $_POST['deleteItem'] as your id
	...
	echo "[";
	for ($i = 0; $i < count($results[1]['acarray']); $i++) {
		if ($results[1]['acarray'][$i] == null)
			break;
		else {
			echo "[";
			echo $results[2]['speed'][$i];
